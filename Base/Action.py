...
class BaseAction:
    ...
    def swipe_right(self):
        ...
        w = win_size["width"]
        h = win_size["height"]
        self.driver.swipe(w * 0.1, h * 0.5, w * 0.9, h * 0.5)
    ...
    # 8 工具方法之向下滑屏
    def swipe_bottom(self, time=1500):
        win_size = self.get_size()
        w = win_size["width"]
        h = win_size["height"]
        self.driver.swipe(w * 0.5, h * 0.1, w * 0.5, h * 0.9, time)
    ...
    def get_ele_ancestry(self, ancestry, sub):
        ...
        # 2 依据祖先定位后代
        try:
            obj = ancestry_obj.find_element(*sub)
        except Exception as e:
            return None
        else:
            return obj
